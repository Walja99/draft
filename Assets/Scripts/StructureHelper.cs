...
namespace SVS
{
	public class StructureHelper : MonoBehaviour
	{
		...
		public IEnumerator PlaceStructuresAroundRoad(List<Vector3> roadPositions)
		{
			Dictionary<Vector3, Direction> freeEstateSpots = FindFreeSpacesAroundRoad(roadPositions);
		...
		private bool VerifyIfBuildingFits(
			int halfSize, 
			Dictionary<Vector3, Direction> freeEstateSpots, 
			KeyValuePair<Vector3, Direction> freeSpot,
		...
		private bool AddTrees()
		{
			for (int i = 0; i < 30; i++)
			{
				for (int j = 0; j < 30; j++)
				{
					Collider[] hitColliders = Physics.OverlapSphere(new Vector3(i,j,0), (float)2);
					
					var building = SpawnPrefab(buildingTypes[1].GetPrefab(), new Vector3(i,j,0), Quaternion.Euler(0, 0, 0));
					
				}
			}
			return true;
		}
		private Dictionary<Vector3, Direction> FindFreeSpacesAroundRoad(List<Vector3> roadPositions)
		{
			Dictionary<Vector3, Direction> freeSpaces = new Dictionary<Vector3, Direction>();
			foreach (var position in roadPositions)
			{
				var neighbourDirections = PlacementHelper.FindNeighbour(position, roadPositions, Quaternion.Euler(0, 0, 0));
